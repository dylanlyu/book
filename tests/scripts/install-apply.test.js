...
function runTests() {
  ...
  if (
    test('supports dry-run without mutating the target project', () => {
      ...
      try {
        ...
        assert.ok(!fs.existsSync(path.join(projectDir, '.agent', 'ecc-install-state.json')));
      } finally {
        cleanup(homeDir);
        cleanup(projectDir);
      }
  ...
  if (
    test('installs antigravity manifest profiles while skipping only unsupported modules', () => {
      ...
      try {
        ...
        assert.strictEqual(state.request.profile, 'core');
        assert.strictEqual(state.request.legacyMode, false);
        assert.deepStrictEqual(state.resolution.selectedModules, ['rules-core', 'agents-core', 'commands-core', 'platform-configs', 'skill-unified-memory', 'workflow-quality']);
  ...
  if (
    test('installs explicit modules for zed using manifest operations', () => {
      const homeDir = createTempDir('install-apply-home-');
      const projectDir = createTempDir('install-apply-project-');

      try {
        const result = run(['--target', 'zed', '--modules', 'platform-configs'], {
          cwd: projectDir,
          homeDir
        });
        assert.strictEqual(result.code, 0, result.stderr);
        assert.ok(fs.existsSync(path.join(projectDir, '.zed', 'scripts', 'auto-update.js')));
        assert.ok(fs.existsSync(path.join(projectDir, '.zed', 'mcp-configs', 'mcp-servers.json')));

        const state = readJson(path.join(projectDir, '.zed', 'ecc-install-state.json'));
