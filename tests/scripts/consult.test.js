...
function runTests() {
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(
      ...
      'npx book-universal install --profile minimal --target claude --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.match(result.stdout, /npx book-universal install --profile minimal --target claude --with capability:security/);
    assert.match(result.stdout, /npx book-universal plan --profile minimal --target claude --with capability:security/);
  })) passed++; else failed++;
