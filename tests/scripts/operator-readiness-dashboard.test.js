...
function seedRepo(rootDir, overrides = {}) {
  const files = {
    'package.json': JSON.stringify(
      {
        ...
        files: ['scripts/operator-readiness-dashboard.js', 'scripts/platform-audit.js', 'scripts/preview-pack-smoke.js', 'scripts/release-video-suite.js'],
        scripts: {
    ...
    'scripts/preview-pack-smoke.js': ['ecc.preview-pack-smoke.v1', 'preview-pack-artifacts-present', 'preview-pack-boundary-sanitized', 'publication-blockers-preserved'].join('\n'),
...
function runTests() {
  ...
  if (
    test('parseArgs accepts dashboard flags and rejects invalid values', () => {
  ...
  if (
    test('seeded repo emits an objective audit with remaining work', () => {
        ...
        assert.ok(
          report.requirements.some(
            item =>
              ...
              item.gap ===
                'repeat KV readback and selected-target announcement gate immediately before launch; keep native-payments copy behind the final release, plugin, URL, and owner-approval gates' &&
  ...
  if (
    test('release video suite moves current when publish-candidate evidence is recorded', () => {
  ...
  if (
    test('Linear progress stays in progress until live sync evidence is mirrored', () => {
  ...
  if (
    test('preview pack gate stays in progress until smoke gate is wired', () => {
      ...
      try {
        seedRepo(rootDir, {
          'package.json': JSON.stringify(
            {
              files: ['scripts/operator-readiness-dashboard.js', 'scripts/platform-audit.js'],
              scripts: {
                'discussion:audit': 'node scripts/discussion-audit.js',
                'operator:dashboard': 'node scripts/operator-readiness-dashboard.js',
                'platform:audit': 'node scripts/platform-audit.js',
  ...
  if (
    test('owner approval packet fails closed when it is missing from the release pack', () => {
  ...
  if (
    test('AgentShield enterprise evidence covers export and policy promotion markers', () => {
      const cases = [
        {
  ...
  if (
    test('legacy salvage recognizes the real manual-review backlog heading', () => {
  ...
  if (
    test('markdown output can be written as the dashboard artifact', () => {
      const rootDir = createTempDir('operator-dashboard-markdown-');
      const outputPath = path.join(rootDir, 'artifacts', 'dashboard.md');

      try {
        seedRepo(rootDir);
        const stdout = run(['--markdown', '--skip-github', `--root=${rootDir}`, '--generated-at=2026-05-15T00:00:00.000Z', '--write', outputPath], { cwd: rootDir });
  ...
  if (
    test('text output renders compact status and top actions', () => {
      const rootDir = createTempDir('operator-dashboard-text-');

      try {
        seedRepo(rootDir);
        const stdout = run(['--format=text', '--skip-github', `--root=${rootDir}`, '--generated-at=2026-05-15T00:00:00.000Z'], { cwd: rootDir });
  ...
  if (
    test('renderers handle a ready report with no top actions', () => {
      const report = {
        dashboardReady: true,
        generatedAt: '2026-05-15T00:00:00.000Z',
        head: 'abc123',
  ...
  if (
    test('exit-code mode fails closed while macro objective has gaps', () => {
      const rootDir = createTempDir('operator-dashboard-exit-');

      try {
        seedRepo(rootDir);
        const result = runProcess(['--json', '--skip-github', `--root=${rootDir}`, '--generated-at=2026-05-15T00:00:00.000Z', '--exit-code'], { cwd: rootDir });
  ...
  if (
    test('cli help exits successfully and invalid cli flags fail before reporting', () => {
