...
function buildExpectedPublishPaths(repoRoot) {
  ...
  const exclusionPaths = ['!**/__pycache__/**', '!**/*.pyc', '!**/*.pyo', '!**/*.pyd', '!**/.pytest_cache/**'];

  const combined = new Set([...modules.flatMap(module => module.paths || []), ...extraPaths, ...exclusionPaths].map(normalizePublishPath));

  return [...combined].filter(publishPath => !isCoveredByAncestor(publishPath, combined)).sort();
}
    ...
    [
      ...
      () => {
        ...
        for (const requiredPath of [
          ...
          assert.ok(packagedPaths.has(requiredPath), `npm pack should include ${requiredPath}`);
        }

        for (const excludedPath of ['contexts/dev.md', 'examples/CLAUDE.md', 'plugins/README.md', 'scripts/ci/catalog.js', 'skills/skill-comply/SKILL.md']) {
          assert.ok(!packagedPaths.has(excludedPath), `npm pack should not include ${excludedPath}`);
        }

        for (const packagedPath of packagedPaths) {
          assert.ok(!packagedPath.includes('__pycache__/'), `npm pack should not include Python bytecode cache path ${packagedPath}`);
          assert.ok(!/\.py[cod]$/.test(packagedPath), `npm pack should not include Python bytecode file ${packagedPath}`);
        }
      }
    ]
  ];
