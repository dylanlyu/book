...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/book', 'win32'),
      'C:/Users/x/.claude/plugins/book'
    );
    assert.strictEqual(
  ...
  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/book', 'darwin'),
      '/c/Users/x/.claude/plugins/book'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/book', 'linux'),
      '/c/Users/x/.claude/plugins/book'
    );
  })) passed++; else failed++;
