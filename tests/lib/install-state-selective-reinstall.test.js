...
try {
  ...
  const result = uninstallInstalledStates({ projectRoot: root, targets: ['claude-project'] });
...
try {
  ...
  const result = uninstallInstalledStates({ projectRoot: partialRoot, targets: ['claude-project'] });
  assert.strictEqual(result.summary.errorCount, 0);
  assert.ok(!fs.existsSync(copied.operations[0].destinationPath));
