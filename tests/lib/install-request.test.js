...
function runTests() {
  ...
  if (test('normalizes manifest installs into a canonical request', () => {
    ...
    assert.strictEqual(request.target, 'claude-project');
  ...
  if (test('rejects the removed --locale flag as an unknown argument', () => {
    assert.throws(
      () => parseInstallArgs(['node', 'scripts/install-apply.js', '--locale', 'zh-TW']),
      /Unknown argument: --locale/
    );
  })) passed++; else failed++;
