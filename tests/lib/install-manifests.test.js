...
function runTests() {
  ...
  if (
    test('loads real project install manifests', () => {
  ...
  if (
    test('lists install profiles from the real project', () => {
      const profiles = listInstallProfiles();
      assert.ok(
        profiles.some(profile => profile.id === 'minimal'),
        'Should include minimal profile'
      );
      assert.ok(
        profiles.some(profile => profile.id === 'core'),
        'Should include core profile'
      );
      assert.ok(
        profiles.some(profile => profile.id === 'full'),
  ...
  if (
    test('lists install modules from the real project', () => {
      const modules = listInstallModules();
      assert.ok(
        modules.some(module => module.id === 'rules-core'),
        'Should include rules-core'
      );
      assert.ok(
        modules.some(module => module.id === 'orchestration'),
  ...
  if (
    test('lists install components from the real project', () => {
      const components = listInstallComponents();
      assert.ok(
        components.some(component => component.id === 'lang:typescript'),
        'Should include lang:typescript'
      );
      assert.ok(
        components.some(component => component.id === 'lang:c'),
        'Should include lang:c'
      );
      assert.ok(
        components.some(component => component.id === 'capability:security'),
        'Should include capability:security'
      );
      assert.ok(
        components.some(component => component.id === 'capability:machine-learning'),
        'Should include capability:machine-learning'
      );
      assert.ok(
        components.some(component => component.id === 'agent:mle-reviewer'),
        'Should include agent:mle-reviewer'
      );
      assert.ok(
        components.some(component => component.id === 'skill:mle-workflow'),
  ...
  if (
    test('gets install component details and validates component IDs', () => {
      ...
      assert.throws(() => getInstallComponent(''), /An install component ID is required/);
      assert.throws(() => getInstallComponent('lang:missing'), /Unknown install component: lang:missing/);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('validates install component filters', () => {
      ...
      assert.throws(() => listInstallComponents({ family: 'unknown' }), /Unknown component family: unknown/);
      assert.throws(() => listInstallComponents({ target: 'unknown-target' }), /Unknown install target: unknown-target/);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('labels continuous-learning as a legacy v1 install surface', () => {
  ...
  if (
    test('exposes continuous-learning-v2 as a single-skill install surface', () => {
  ...
  if (
    test('marks unified-memory install surfaces as requiring the separate ECC runtime', () => {
      const component = getInstallComponent('skill:unified-memory');
      assert.deepStrictEqual(component.moduleIds, ['skill-unified-memory']);
      assert.match(component.description, /book-universal/i);
      ...
      assert.match(singleSkillModule.description, /book-universal/i);
      assert.match(singleSkillModule.description, /separate|external/i);
      assert.match(workflowModule.description, /book-universal/i);
  ...
  if (
    test('lists supported legacy compatibility languages', () => {
  ...
  if (
    test('resolves minimal profile without the hook runtime', () => {
      const plan = resolveInstallPlan({
        profileId: 'minimal',
        target: 'claude',
      ...
      assert.ok(!plan.selectedModuleIds.includes('hooks-runtime'), 'minimal profile should not install hooks-runtime');
  ...
  if (
    test('resolves machine-learning component with workflow dependencies', () => {
      ...
      assert.ok(plan.selectedModuleIds.includes('machine-learning'), 'Should include machine-learning module');
      assert.ok(plan.selectedModuleIds.includes('framework-language'), 'Should include Python and framework-language support');
      assert.ok(plan.selectedModuleIds.includes('workflow-quality'), 'Should include eval and verification workflows');
      assert.ok(plan.selectedModuleIds.includes('database'), 'Should include database/data persistence support');
      assert.ok(plan.selectedModuleIds.includes('devops-infra'), 'Should include deployment and container support');
      assert.ok(plan.selectedModuleIds.includes('security'), 'Should include security through machine-learning dependencies');
      assert.ok(
        plan.operations.some(operation => operation.sourceRelativePath === 'skills/mle-workflow'),
  ...
  if (
    test('resolves machine-learning component on the claude-project target', () => {
      for (const target of ['claude-project']) {
        ...
        assert.ok(plan.selectedModuleIds.includes('machine-learning'), `Should include machine-learning module for ${target}`);
        assert.ok(!plan.skippedModuleIds.includes('machine-learning'), `Should not skip machine-learning module for ${target}`);
        assert.ok(
          plan.operations.some(operation => operation.sourceRelativePath === 'skills/mle-workflow'),
  ...
  if (
    test('minimal machine-learning install includes MLE reviewer agent surface', () => {
      const plan = resolveInstallPlan({
        profileId: 'minimal',
        includeComponentIds: ['capability:machine-learning'],
        target: 'claude',
      ...
      assert.ok(plan.selectedModuleIds.includes('agents-core'), 'Minimal install should keep the agent surface available');
      assert.ok(
        plan.operations.some(operation => operation.sourceRelativePath === 'agents'),
  ...
  if (
    test('resolves explicit modules with dependency expansion', () => {
      ...
      assert.ok(plan.selectedModuleIds.includes('workflow-quality'), 'Should include transitive dependency');
      assert.ok(plan.selectedModuleIds.includes('platform-configs'), 'Should include nested dependency');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('validates explicit module IDs against the real manifest catalog', () => {
      ...
      assert.throws(() => validateInstallModuleIds(['ghost-module']), /Unknown install module: ghost-module/);
      assert.throws(() => validateInstallModuleIds(['ghost-one', 'ghost-two']), /Unknown install modules: ghost-one, ghost-two/);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolves legacy compatibility selections into manifest module IDs', () => {
  ...
  if (
    test('resolves rust legacy compatibility into framework-language module', () => {
      ...
      assert.ok(selection.moduleIds.includes('rules-core'));
      assert.ok(selection.moduleIds.includes('framework-language'), 'rust should resolve to framework-language module');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolves cpp legacy compatibility into framework-language module', () => {
      ...
      assert.ok(selection.moduleIds.includes('rules-core'));
      assert.ok(selection.moduleIds.includes('framework-language'), 'cpp should resolve to framework-language module');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolves c legacy compatibility into framework-language module', () => {
      ...
      assert.ok(selection.moduleIds.includes('rules-core'));
      assert.ok(selection.moduleIds.includes('framework-language'), 'c should resolve to framework-language module');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolves csharp legacy compatibility into framework-language module', () => {
      ...
      assert.ok(selection.moduleIds.includes('rules-core'));
      assert.ok(selection.moduleIds.includes('framework-language'), 'csharp should resolve to framework-language module');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolves fsharp legacy compatibility into framework-language module', () => {
      ...
      assert.ok(selection.moduleIds.includes('rules-core'));
      assert.ok(selection.moduleIds.includes('framework-language'), 'fsharp should resolve to framework-language module');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('rejects unknown legacy compatibility languages', () => {
      assert.throws(
        () =>
          resolveLegacyCompatibilitySelection({
            target: 'claude-project',
  ...
  if (
    test('resolves included and excluded user-facing components', () => {
      const plan = resolveInstallPlan({
        profileId: 'core',
      ...
      assert.ok(plan.excludedModuleIds.includes('orchestration'), 'Should report modules removed by excluded components');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('fails when a selected component depends on an excluded component module', () => {
      assert.throws(
        () =>
          resolveInstallPlan({
  ...
  if (
    test('throws on unknown install profile', () => {
      assert.throws(() => resolveInstallPlan({ profileId: 'ghost-profile' }), /Unknown install profile/);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('throws on unknown install target', () => {
      assert.throws(() => resolveInstallPlan({ profileId: 'core', target: 'not-a-target' }), /Unknown install target/);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('rejects empty, unknown, and fully excluded install selections', () => {
      ...
      try {
        ...
        assert.throws(() => resolveInstallPlan({ repoRoot }), /No install profile, module IDs, or included component IDs were provided/);
        assert.throws(() => resolveInstallPlan({ repoRoot, moduleIds: ['missing'] }), /Unknown install module: missing/);
        assert.throws(() => resolveInstallPlan({ repoRoot, includeComponentIds: ['capability:missing'] }), /Unknown install component: capability:missing/);
        assert.throws(
          () =>
            resolveInstallPlan({
              repoRoot,
  ...
  if (
    test('skips a requested module when its dependency chain does not support the target', () => {
      const repoRoot = createTestRepo();
      try {
        writeJson(path.join(repoRoot, 'manifests', 'install-modules.json'), {
          version: 1,
  ...
  if (
    test('rejects missing, malformed, and unsupported manifest fixtures', () => {
      const repoRoot = createTestRepo();
      try {
        assert.throws(() => loadInstallManifests({ repoRoot }), /Install manifests not found/);
        ...
        assert.throws(() => loadInstallManifests({ repoRoot }), /Failed to read install-modules\.json/);
        ...
        assert.throws(() => loadInstallManifests({ repoRoot }), /Install module empty-target has invalid targets/);
        ...
        assert.throws(() => loadInstallManifests({ repoRoot }), /Install module unsupported-target has unsupported targets: moonbase/);
  ...
  if (
    test('fails fast when install manifest module targets is not an array', () => {
      const repoRoot = createTestRepo();
      try {
        writeJson(path.join(repoRoot, 'manifests', 'install-modules.json'), {
          version: 1,
        ...
        assert.throws(() => resolveInstallPlan({ repoRoot, profileId: 'core', target: 'claude' }), /Install module parent has invalid targets; expected an array of supported target ids/);
      } finally {
        cleanupTestRepo(repoRoot);
      }
    })
  ...
  if (
    test('detects circular install dependencies', () => {
      const repoRoot = createTestRepo();
      try {
        writeManifestSet(repoRoot, {
          modules: [
        ...
        assert.throws(() => resolveInstallPlan({ repoRoot, profileId: 'core' }), /Circular install dependency detected at alpha/);
      } finally {
        cleanupTestRepo(repoRoot);
      }
    })
