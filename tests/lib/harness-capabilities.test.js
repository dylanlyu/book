...
const { GUIDED_HARNESS_IDS, HARNESS_CAPABILITIES, getHarnessCapability, listGuidedHarnesses, listHarnessCapabilities, normalizeHarnessSelection } = require('../../scripts/lib/harness-capabilities');
...
function runTests() {
  ...
  if (
    test('represents all 3 registered targets exactly once across 2 harnesses', () => {
      ...
      assert.strictEqual(HARNESS_CAPABILITIES.length, 2);
      assert.strictEqual(new Set(catalogTargetIds).size, 3);
  ...
  if (
    test('every catalogued harness is guided-ready', () => {
      assert.deepStrictEqual(GUIDED_HARNESS_IDS, ['claude', 'codex']);
      assert.deepStrictEqual(
        listGuidedHarnesses().map(harness => harness.id),
        ['claude', 'codex']
      );
      assert.ok(HARNESS_CAPABILITIES.every(harness => harness.guidedReady === true));
      assert.ok(HARNESS_CAPABILITIES.every(harness => harness.availability === 'guided'));
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('models reviewed guided install modes, roots, and scopes', () => {
      ...
      assert.deepStrictEqual(codex.scopes, [{ id: 'native', targetId: 'codex', root: '~/.codex' }]);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('describes hook capability for each guided harness', () => {
  ...
  if (
    test('does not advertise unregistered or removed harnesses', () => {
      for (const id of ['copilot', 'kiro', 'pi', 'cursor', 'opencode', 'kimi', 'antigravity', 'joycode']) {
        assert.strictEqual(getHarnessCapability(id), null);
        assert.throws(() => normalizeHarnessSelection(id), /Unknown guided harness selection/);
      }
    })
  )
    passed++;
  ...
  if (
    test('normalizes wizard selections into canonical guided order', () => {
      assert.deepStrictEqual(normalizeHarnessSelection(' Claude Code, claude '), ['claude']);
      assert.deepStrictEqual(normalizeHarnessSelection(['2', 'claude-project', 'Codex']), ['claude', 'codex']);
  ...
  if (
    test('rejects empty, ambiguous, and unknown wizard selections clearly', () => {
  ...
  if (
    test('exports deeply frozen records while list helpers return safe array copies', () => {
      ...
      first.pop();
      assert.strictEqual(listHarnessCapabilities().length, 2);
