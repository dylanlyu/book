...
function runTests() {
  ...
  if (
    test('getHomeDir prefers HOME override when set', () => {
  ...
  if (
    test('getHomeDir falls back to USERPROFILE when HOME is empty', () => {
      const originalHome = process.env.HOME;
      const originalUserProfile = process.env.USERPROFILE;
  ...
  if (
    test('getAgentDataHome honors ECC_AGENT_DATA_HOME', () => {
      ...
      try {
        ...
        assert.strictEqual(reloaded.getSessionsDir(), path.join(path.resolve(overrideRoot), 'session-data'));
        assert.strictEqual(reloaded.getLearnedSkillsDir(), path.join(path.resolve(overrideRoot), 'skills', 'learned'));
      } finally {
        delete require.cache[require.resolve('../../scripts/lib/utils')];
  ...
  if (
    test('getAgentDataHome expands tilde in ECC_AGENT_DATA_HOME', () => {
      const original = process.env.ECC_AGENT_DATA_HOME;
      try {
        process.env.ECC_AGENT_DATA_HOME = path.join('~', '.claude', 'ecc-test');
        delete require.cache[require.resolve('../../scripts/lib/utils')];
        const reloaded = require('../../scripts/lib/utils');
        const expected = path.join(reloaded.getHomeDir(), '.claude', 'ecc-test');
  ...
  if (
    test('getSessionSearchDirs includes canonical and legacy paths', () => {
  ...
  if (
    test('sanitizeSessionId strips leading dots', () => {
  ...
  if (
    test('sanitizeSessionId replaces dots and spaces', () => {
  ...
  if (
    test('sanitizeSessionId replaces special chars and collapses runs', () => {
  ...
  if (
    test('sanitizeSessionId preserves valid chars', () => {
  ...
  if (
    test('sanitizeSessionId appends hash suffix for all Windows reserved device names', () => {
  ...
  if (
    test('sanitizeSessionId returns null for empty or punctuation-only values', () => {
  ...
  if (
    test('sanitizeSessionId returns stable hashes for non-ASCII values', () => {
  ...
  if (
    test('sanitizeSessionId disambiguates mixed-script names from pure ASCII', () => {
  ...
  if (
    test('sanitizeSessionId is idempotent', () => {
  ...
  if (
    test('sanitizeSessionId preserves readable prefixes for Windows reserved device names', () => {
  ...
  if (
    test('getSessionIdShort falls back to sanitized project name', () => {
  ...
  if (
    test('getSessionIdShort sanitizes explicit fallback parameter', () => {
  ...
  if (
    test('readStdinJson overflow drain still exits when the writer never closes stdin', () => {
      const { execFileSync } = require('child_process');
      const childScript = ['const u=require("./scripts/lib/utils");', 'u.readStdinJson({timeoutMs:100,maxSize:100})', '.then(d=>process.stdout.write(JSON.stringify(d)));'].join('');
      const harness = `
  ...
  if (
    test('readStdinJson drains a slow finite oversized writer without EPIPE', () => {
      const { execFileSync } = require('child_process');
      const childScript = ['const u=require("./scripts/lib/utils");', 'u.readStdinJson({timeoutMs:500,maxSize:100})', '.then(d=>process.stdout.write(JSON.stringify(d)));'].join('');
      const harness = `
      const { spawn } = require('child_process');
      const child = spawn(process.execPath, ['-e', ${JSON.stringify(childScript)}], {
        cwd: process.cwd(),
  ...
  if (
    test('runCommand returns error output on failed command', () => {
  ...
  if (
    test('runCommand blocks disallowed command prefix', () => {
  ...
  if (
    test('runCommand blocks curl command', () => {
  ...
  if (
    test('runCommand blocks bash command', () => {
  ...
  if (
    test('runCommand blocks semicolon command chaining', () => {
  ...
  if (
    test('runCommand blocks pipe command chaining', () => {
  ...
  if (
    test('runCommand blocks ampersand command chaining', () => {
      const result = utils.runCommand('git status && echo pwned');
      assert.strictEqual(result.success, false);
  ...
  if (
    test('runCommand blocks dollar sign command substitution', () => {
  ...
  if (
    test('runCommand blocks backtick command substitution', () => {
      const result = utils.runCommand('git log `whoami`');
      assert.strictEqual(result.success, false);
  ...
  if (
    test('runCommand allows metacharacters inside double quotes', () => {
  ...
  if (
    test('runCommand allows metacharacters inside single quotes', () => {
  ...
  if (
    test('runCommand blocks unquoted metacharacters alongside quoted ones', () => {
  ...
  if (
    test('runCommand blocks prefix without trailing space', () => {
  ...
  if (
    test('runCommand allows npx prefix', () => {
  ...
  if (
    test('runCommand blocks newline command injection', () => {
  ...
  if (
    test('runCommand blocks $() inside double quotes (shell still evaluates)', () => {
  ...
  if (
    test('runCommand blocks backtick inside double quotes (shell still evaluates)', () => {
      const result = utils.runCommand('node -e "`whoami`"');
      assert.strictEqual(result.success, false);
  ...
  if (
    test('runCommand error message does not leak command string', () => {
  ...
  if (
    test('getSessionIdShort sanitizes whitespace-only CLAUDE_SESSION_ID to fallback', () => {
      if (process.platform === 'win32') {
        console.log('    (skipped — root CWD differs on Windows)');
        return true;
      }
  ...
  if (
    test('strips SGR color codes (\\x1b[...m)', () => {
  ...
  if (
    test('strips cursor movement sequences (\\x1b[H, \\x1b[2J, \\x1b[3J)', () => {
  ...
  if (
    test('strips cursor position sequences (\\x1b[row;colH)', () => {
  ...
  if (
    test('strips erase line sequences (\\x1b[K, \\x1b[2K)', () => {
  ...
  if (
    test('strips charset selection (\\x1b(B)', () => {
  ...
  if (
    test('strips bare ESC + letter (\\x1bM reverse index)', () => {
  ...
  if (
    test('handles mixed ANSI sequences in one string', () => {
  ...
  if (
    test('preserves string with no ANSI codes', () => {
  ...
  if (
    test('handles CSI with question mark parameter (DEC private modes)', () => {
