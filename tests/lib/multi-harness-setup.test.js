...
(async () => {
  ...
  await test('normalizes provider-specific options without inventing shared semantics', () => {
    ...
    assert.throws(
      () => normalizeGuidedInstallRequest({ harnesses: ['codex'], claudeScope: 'user' }),
      /Claude.*selected/i
    );
    assert.throws(
      () => normalizeGuidedInstallRequest({ harnesses: ['codex'], profile: 'core' }),
      /No guided harness accepts a managed install profile/i
    );
  });
  ...
  await test('rejects valid install-state from a different managed target identity', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        /install-state.*target identity|does not belong.*managed/i
      );
  ...
  await test('rejects an unwritable managed destination during preflight', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        error => (
          /Managed install destination is not writable by the current user/i.test(error.message)
  ...
  await test('preflights every selected harness before applying any mutation', async () => {
    ...
    await assert.rejects(
      () => createMultiHarnessPlan(request, {
        ...
        previewCodex: async () => {
          events.push('preview:codex');
          throw new Error('unowned collision');
        },
      }),
      /collision/
    );
    assert.deepStrictEqual(events, ['preview:claude', 'preview:codex']);
  });
