...
2. Preserve existing project guidance. If `CLAUDE.md`, `.claude/settings.local.json`, `.codex/`, `.opencode/`, `.joycode/`, `.zed/`, or `.qwen/` already exists, inspect it and propose a merge/append plan instead of overwriting.
...
1. Identify the target harness. Default to `claude` unless the user asks for `codex`, `opencode`, `joycode`, `zed`, or `qwen`.
