...
function normalizeGuidedInstallRequest(input = {}) {
  ...
  if (harnesses.length === 0) {
    throw new Error('Choose at least one guided harness: Claude or Codex.');
  }
  ...
  if (input.profile !== undefined) {
    throw new Error('No guided harness accepts a managed install profile.');
  }
...
function assertPriorInstallStateMatchesPlan(state, plan) {
  ...
  if (
    ...
    throw new Error(
      ...
      + 'target identity does not match the current managed install plan.'
    );
  }
...
async function createMultiHarnessPlan(request, injected = {}) {
...
async function applyMultiHarnessPlan(plan, injected = {}) {
  const dependencies = { ...defaultDependencies(), ...injected };
