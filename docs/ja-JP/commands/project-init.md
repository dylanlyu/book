...
2. 既存のプロジェクトガイダンスを保持。`CLAUDE.md`、`.claude/settings.local.json`、`.codex/`が既に存在する場合、内容を検査し上書きではなくマージ/追記計画を提案。
...
1. ターゲットハーネスを特定。ユーザーが`claude-project`、`antigravity`、または`codex`を要求しない限りデフォルトは`claude`。
